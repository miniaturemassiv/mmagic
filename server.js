      ...
      try {
        ...
        if (mediaResponse.ok) {
          ...
          if (attachRes.ok) {
            ...
            console.error('❌ Failed to attach featured image (Post created without image):', attachErr);
          }
        } else {
          const errText = await mediaResponse.text();
          console.error('❌ Media upload failed (Post created without image):', errText);
        }
        fs.unlinkSync(lastImagePath);
      } catch (imgErr) {
        console.error('❌ Image processing error (Post created without image):', imgErr.message);
      }
    }
